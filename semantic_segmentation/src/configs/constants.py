...
if SECRET_KEY:
    ...
    PACKAGE_PATH = '/home/daniel/Documents/hsi-forest-segmentation/Kyselica/semantic_segmentation'
...
# MASK_DIFFERENCE_INDEX - C  in mask suggest that original mask does not contain C class at particular pixel
...
CROSS_ENTROPY_LOSS = LossConfig(
    name=LossFunctions.CROSS_ENTROPY,
    args={
        'ignore_index': IGNORE_INDDEX,
    }
)

IOU_LOSS = LossConfig(
    name=LossFunctions.IOU,
    args={
        'ignore_index': IGNORE_INDDEX,
        'n_classes' : 2,
        'bakground_class': False,
    }
)

FOCAL_LOSS = LossConfig(
    name=LossFunctions.FOCAL,
    args={
        'gamma': 0.5,
        'ingnore_index': IGNORE_INDDEX
    }
)

DEFAULT_NET_CONFIG = NetConfig(
    architecture=NetworkArchitectures.UNET,
    loss_cfg=CROSS_ENTROPY_LOSS,
    args={
