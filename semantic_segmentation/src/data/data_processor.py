...
class DataProcessor:
    ...
    def _load_data_folder(self, folder):
        ...
        """
        def sort_key(path):
            return tuple(map(int, re.findall(r"\d+", os.path.split(path)[-1])))
        
        
        print(f"Loading: {folder}")
        
        all_images = sorted(glob.glob(f"{folder}/images/*.tif"), key=sort_key)
        all_masks = sorted(glob.glob(f"{folder}/masks/*.tif"), key=sort_key)
        ...
        for img, mask in zip(all_images, all_masks):
            assert sort_key(img) == sort_key(mask) # Must correspond to the same patch
    ...
    def _split_train_val_indices(self, N, val_size=0.2, random_state=42):
        ...
        # to compensate if the images in the dataset are numbered from 1
        indices += self.first_id
    ...
    def _compute_mean_std(self, X_path_list, y_path_list, scale, width, height, batch_size=32):
        ...
        transforms = self._get_transformations(mode='basic')
    ...
    def _get_transformations(self, mode='basic'):
        ...
        return A.Compose(transformation_list, is_check_shapes=False)
    ...
    def get_pytorch_datasets(self):
        ...
        if self.train_set is None or self.val_set is None:
            ...
            t_train = self._get_transformations(mode='train')
            t_val   = self._get_transformations(mode='val')
